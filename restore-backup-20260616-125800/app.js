...
function auditManagement(action, recordType = "management", recordId = null, beforeData = null, afterData = null) {
  window.logManagementAction?.(action, recordType, recordId, beforeData, afterData);
}
window.auditManagement = auditManagement;

$("#cartOpen").onclick = () => setDrawer(true);
$("#drawerBackdrop").onclick = () => setDrawer(false);
$('[data-close="cart"]').onclick = () => setDrawer(false);
$("#checkoutOpen").onclick = () => { setDrawer(false); openModal("#checkoutModal"); };
$("#searchInput").oninput = renderProducts;
$("#legalNoticeOpen").onclick = () => openModal("#legalModal");
$("#legalAcceptance").onchange = event => {
...
$("#adminOpen").onclick = () => window.requestManagementAccess?.();
$("#adminClose").onclick = () => $("#adminShell").classList.remove("open");
...
$("#feeSettingsForm").onsubmit = event => {
  event.preventDefault();
  const form = event.target;
  const data = Object.fromEntries(new FormData(form));
